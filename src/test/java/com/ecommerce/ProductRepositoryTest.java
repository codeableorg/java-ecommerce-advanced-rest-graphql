...
class ProductRepositoryTest {
        ...
        void findByNameContainingIgnoreCase_devuelveProductos() {
                databaseClient.sql(
                                "INSERT INTO products (id,name,description,category,price,available,sales) VALUES (500,'abc','desc','cat',10.0,true,1)")
                                .fetch().rowsUpdated().block();
                StepVerifier.create(productRepository.findByNameContainingIgnoreCase("a",
                                org.springframework.data.domain.Pageable.unpaged()))
                                .expectNextMatches(p -> p.getName().contains("a") ||
